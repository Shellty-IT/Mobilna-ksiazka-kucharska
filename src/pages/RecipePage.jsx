...
export function RecipePage() {
  ...
  return <main className="page section-wrap recipe-view"><Link className="back-link" to="/szukaj">← Wróć do wyników</Link><header className="recipe-hero"><p className="eyebrow">Przepis {String(recipe.number).padStart(2, "0")}</p><h1>{recipe.name}</h1><div className="recipe-meta"><span>{(recipe.composition || []).length} składników</span>{recipe.prepTime && <span>około {recipe.prepTime} min</span>}</div><p>Przygotuj danie krok po kroku. Zaznaczaj składniki, które masz już przygotowane.</p></header><div className="recipe-layout"><section className="ingredient-list"><h2>Składniki</h2><ul>{(recipe.composition || []).map((ingredient, index) => <li key={`${ingredient}-${index}`}><label><input type="checkbox" /><span>{ingredient}</span></label></li>)}</ul></section><section className="method"><h2>Przygotowanie</h2>{steps.map((step, index) => <article className="method-step" key={`${step}-${index}`}><span>{String(index + 1).padStart(2, "0")}</span><p>{step}</p></article>)}</section></div></main>;
}
